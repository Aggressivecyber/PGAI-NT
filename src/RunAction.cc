...
namespace {
std::once_flag gBookNtupleFlag;
std::atomic<bool> gNtupleBooked{ false };
}
...
void RunAction::BeginOfRunAction(const G4Run* aRun) {
        auto man = G4AnalysisManager::Instance();
        std::call_once(gBookNtupleFlag, [man]() {
...
bool RunAction::IsNtupleBooked()
{
        return gNtupleBooked.load();
}
